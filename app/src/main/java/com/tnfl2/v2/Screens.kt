...
// ─────────────────────────────────────────────────────────────────────────────
// POS SCREEN
// Point-of-Sale quick order entry using live product data
// ─────────────────────────────────────────────────────────────────────────────
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun POSScreen(token: String, showCartTrigger: Boolean = false, onCartHandled: () -> Unit = {}) {
    val authRepository = remember { com.tnfl2.v2.network.AuthRepository() }
    val scope = rememberCoroutineScope()
    val context = LocalContext.current

    var products by remember { mutableStateOf<List<com.tnfl2.v2.network.Product>>(emptyList()) }
    var isLoading by remember { mutableStateOf(true) }
    var searchQuery by remember { mutableStateOf("") }
    val orderItems = remember { mutableStateListOf<Pair<com.tnfl2.v2.network.Product, Int>>() }
    val currencyFormat = remember { java.text.NumberFormat.getCurrencyInstance(java.util.Locale.forLanguageTag("en-IN")) }

    var showCartSheet by remember { mutableStateOf(false) }
    var paymentMethod by remember { mutableStateOf("Cash") } // Cash or Digital

    LaunchedEffect(showCartTrigger) {
        if (showCartTrigger) {
            showCartSheet = true
            onCartHandled()
        }
    ...
    LaunchedEffect(token) {
        ...
        authRepository.getProductMaster(token).fold(
            onSuccess = { products = it.productList }, // Removed filter to show 0 stock
            onFailure = { Toast.makeText(context, "Failed to load products: ${it.message}", Toast.LENGTH_SHORT).show() }
        )
        isLoading = false
    ...
    val filteredProducts = products.filter {
        it.sku.contains(searchQuery, ignoreCase = true) || it.brand.contains(searchQuery, ignoreCase = true)
    }
    
    val orderSubtotal = orderItems.sumOf { (p, qty) -> p.salePrice * qty }
    val orderTax = orderSubtotal * 0.05 // 5% tax based on screenshot
    val orderTotal = orderSubtotal + orderTax
    val totalQty   = orderItems.sumOf { it.second }
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            if (!isLoading) {
                LazyVerticalGrid(
                    ...
                    items(filteredProducts) { product ->
                        val qtyInCart = orderItems.firstOrNull { it.first.sku == product.sku }?.second ?: 0
                        val firstLetter = product.category.firstOrNull()?.uppercase() ?: "P"
                        ...
                        Card(
                            ...
                            Column(Modifier.padding(12.dp)) {
                                // Top box
                                Box(
                                    modifier = Modifier.fillMaxWidth().height(60.dp).background(Color(0xFFE0F2F1), RoundedCornerShape(8.dp)),
                                    contentAlignment = Alignment.Center
                                ) {
                                    Text(firstLetter, color = Color(0xFF80CBC4), fontSize = 28.sp, fontWeight = FontWeight.ExtraBold)
                                }
                                
                                Spacer(Modifier.height(12.dp))
                                
                                Text(product.category.uppercase(Locale.ROOT), color = Color(0xFF1E3A8A), fontSize = 12.sp, fontWeight = FontWeight.Bold, maxLines = 1)
                                Spacer(Modifier.height(4.dp))
                                Text(product.sku.uppercase(Locale.ROOT), color = Color(0xFF94A3B8), fontSize = 10.sp, maxLines = 1, overflow = androidx.compose.ui.text.style.TextOverflow.Ellipsis)
                                ...
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.Bottom) {
                                    Column {
                                        Text(currencyFormat.format(product.salePrice), color = Color(0xFF0F766E), fontSize = 16.sp, fontWeight = FontWeight.ExtraBold)
                                        Spacer(Modifier.height(4.dp))
                                        val stockColor = if (product.stock <= 5) Color(0xFFC53030) else Color(0xFF2E7D32)
                                        Text("${product.stock} in stock", color = stockColor, fontSize = 10.sp)
                                    }
                                    
                                    Box(
                                        modifier = Modifier.size(28.dp).background(Color(0xFF0F766E), CircleShape).clickable(enabled = inStock) {
                                            val idx = orderItems.indexOfFirst { it.first.sku == product.sku }
                                            if (idx >= 0) orderItems[idx] = orderItems[idx].copy(second = orderItems[idx].second + 1)
                                            else orderItems.add(Pair(product, 1))
                                        },
                                        contentAlignment = Alignment.Center
                                    ) {
    ...
    if (showCartSheet) {
        ModalBottomSheet(
            ...
            Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 24.dp).padding(bottom = 24.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(Icons.Default.ShoppingCart, contentDescription = null, tint = Color(0xFF007B83), modifier = Modifier.size(20.dp))
                    Spacer(Modifier.width(8.dp))
                    Text("Cart", color = Color(0xFF004455), fontSize = 18.sp, fontWeight = FontWeight.Bold)
                }
                ...
                if (orderItems.isEmpty()) {
                    Box(modifier = Modifier.fillMaxWidth().height(150.dp), contentAlignment = Alignment.Center) {
                        Text("Cart is empty", color = Color(0xFF94A3B8), fontSize = 16.sp)
                    }
                } else {
                        ...
                        items(orderItems) { (product, qty) ->
                            Row(modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                                Column(modifier = Modifier.weight(1f)) {
                                    Text(product.sku, color = MaterialTheme.colorScheme.onSurface, fontSize = 14.sp, fontWeight = FontWeight.Bold)
                                    Text(currencyFormat.format(product.salePrice), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 12.sp)
                                }
                                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.background(Color(0xFFF1F5F9), RoundedCornerShape(16.dp))) {
                                    IconButton(
                                        onClick = {
                                            val idx = orderItems.indexOfFirst { it.first.sku == product.sku }
                                            if (idx >= 0) { if (orderItems[idx].second > 1) orderItems[idx] = orderItems[idx].copy(second = orderItems[idx].second - 1) else orderItems.removeAt(idx) }
                                        },
                                        modifier = Modifier.size(32.dp)
                                    ) { Icon(Icons.Default.Remove, contentDescription = null, tint = Color(0xFF0F766E), modifier = Modifier.size(16.dp)) }
                                    Text("$qty", color = MaterialTheme.colorScheme.onSurface, fontSize = 14.sp, fontWeight = FontWeight.Bold, modifier = Modifier.padding(horizontal = 8.dp))
                                    IconButton(
                                        onClick = {
                                            val idx = orderItems.indexOfFirst { it.first.sku == product.sku }
                                            if (idx >= 0) orderItems[idx] = orderItems[idx].copy(second = orderItems[idx].second + 1)
                ...
                Card(
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            Text("Subtotal", color = Color(0xFF94A3B8), fontSize = 14.sp)
                            Text(currencyFormat.format(orderSubtotal), color = Color(0xFF004455), fontSize = 14.sp, fontWeight = FontWeight.Bold)
                        }
                        Spacer(Modifier.height(8.dp))
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            Text("Tax (5%)", color = Color(0xFF94A3B8), fontSize = 14.sp)
                            Text(currencyFormat.format(orderTax), color = Color(0xFF004455), fontSize = 14.sp, fontWeight = FontWeight.Bold)
                        }
                        Spacer(Modifier.height(16.dp))
                        HorizontalDivider(color = Color(0xFFF1F5F9))
                        Spacer(Modifier.height(16.dp))
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                            Text("Total", color = Color(0xFF004455), fontSize = 16.sp, fontWeight = FontWeight.ExtraBold)
                            Text(currencyFormat.format(orderTotal), color = Color(0xFF00B4A0), fontSize = 18.sp, fontWeight = FontWeight.ExtraBold)
                        }
                        ...
                        Row(
                            modifier = Modifier.fillMaxWidth().height(44.dp).border(1.dp, Color(0xFFF1F5F9), RoundedCornerShape(8.dp)).clip(RoundedCornerShape(8.dp))
                        ) {
                            Box(
                                modifier = Modifier.weight(1f).fillMaxHeight().background(if (paymentMethod == "Cash") Color(0xFF00A090) else Color.White).clickable { paymentMethod = "Cash" },
                                contentAlignment = Alignment.Center
                            ) {
                                Text("Cash", color = if (paymentMethod == "Cash") Color.White else Color(0xFF0F172A), fontWeight = FontWeight.Bold, fontSize = 13.sp)
                            }
                            Box(
                                modifier = Modifier.weight(1f).fillMaxHeight().background(if (paymentMethod == "Digital") Color(0xFF00A090) else Color.White).clickable { paymentMethod = "Digital" },
                                contentAlignment = Alignment.Center
                            ) {
                                Text("Digital", color = if (paymentMethod == "Digital") Color.White else Color(0xFF0F172A), fontWeight = FontWeight.Bold, fontSize = 13.sp)
                            }
                        }
                        ...
                        Button(
                            onClick = { 
                                orderItems.clear()
                                showCartSheet = false
                                Toast.makeText(context, "Order completed via $paymentMethod", Toast.LENGTH_SHORT).show()
                            },
                            modifier = Modifier.fillMaxWidth().height(48.dp),
                            shape = RoundedCornerShape(8.dp),
                            colors = ButtonDefaults.buttonColors(
                                containerColor = if (orderItems.isNotEmpty()) Color(0xFF0F766E) else Color(0xFF91D0C1),
                                disabledContainerColor = Color(0xFF91D0C1).copy(alpha = 0.5f)
                            ),
                            enabled = orderItems.isNotEmpty()
                        ) {
                            Text("CHECKOUT", color = Color.White, fontWeight = FontWeight.Bold, letterSpacing = 1.sp)
                        }
                    }
                }
            }
